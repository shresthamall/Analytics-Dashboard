  ...
  try {
    ...
    if (!type || !country) {
      wsServer.broadcastAlert(
        "warning",
        "Invalid event data: type and country are required",
        {
    ...
    if (!validTypes.includes(type)) {
      wsServer.broadcastAlert(
        "warning",
        `Invalid event type: "${type}". Valid types are: ${validTypes.join(
          ", "
        )}`,
        {
          invalidType: type,
    ...
    if (type === "pageview" || type === "click") {
      ...
      const duration = Math.floor(
        (new Date(event.timestamp) - new Date(session.startTime)) / 1000
      );
    ...
    const recentEvents = store.visitorEvents.filter(
      (event) => new Date(event.timestamp) > new Date(Date.now() - 60 * 1000) // Last minute
    );
    ...
    wsServer.broadcastAlert(
      "warning",
      "Error processing visitor event. Please check the data format.",
      {
        error: error.message,
        receivedData: req.body,
      }
...
app.get("/api/analytics/summary", (req, res) => {
  try {
    ...
    const recentEvents = store.visitorEvents.filter(
      (event) => new Date(event.timestamp) > tenMinutesAgo
    );
    ...
    wsServer.broadcastAlert(
      "warning",
      "Error fetching analytics summary. Please try again.",
      {
        error: error.message,
      }
    );
...
app.get("/api/analytics/sessions", (req, res) => {
  try {
    const activeSessionsList = Array.from(store.activeSessions.values()).map(
      (session) => {
    ...
    wsServer.broadcastAlert(
      "warning",
      "Error fetching active sessions. Please try again.",
      {
        error: error.message,
      }
    );
...
app.post("/api/analytics/clear", (req, res) => {
  try {
    ...
    wsServer.broadcastAlert(
      "warning",
      "Error clearing analytics. Please try again.",
      {
        error: error.message,
      }
    );
